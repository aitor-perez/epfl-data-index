...
- **models/** — Pydantic models: `Document` base class, `Publication`, `Professor`, `Unit`, plus nested variants for many-to-many relations.
- **search.py** — Public API surface (`search`, `fetch_all`, `knn`, `embed`). `search` performs neural (embedding) search over `text` field. `fetch_all` uses point-in-time (PIT) pagination. `knn` finds nearest neighbors by document ID.
